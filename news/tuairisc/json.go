...
// RSS structures for parsing Tuairisc XML feeds (yes, I am just trying to replicate France24's feeds)
type RSS struct {
...
type tuairisc struct {
	oldArticleTitles []string
}

func (a *tuairisc) getArticles(url string, topic news.Topic) ([]news.Article, error) {
	// Fetch RSS XML
...
func (a *tuairisc) getFullArticle(articleURL string) (*string, *news.Location, *news.Thumbnail, error) {
	if articleURL == "" {
...
func (a *tuairisc) extractArticleBody(html string) (*string, error) {
...
func (a *tuairisc) extractLocationFromContent(html string) *news.Location {
	doc, err := goquery.NewDocumentFromReader(strings.NewReader(html))
	if err != nil {
...
func (a *tuairisc) extractThumbnail(html string) *news.Thumbnail {
	doc, err := goquery.NewDocumentFromReader(strings.NewReader(html))
	if err != nil {
		log.Println("Failed to parse HTML:", err)
		return nil
	...
	if strings.HasPrefix(imageURL, "//") {
		...
		imageURL = "https://www.tuairisc.ie" + imageURL
	}
